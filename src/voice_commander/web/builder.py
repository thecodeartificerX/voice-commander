...
from voice_commander.commands.graph_schema import CURRENT_SCHEMA_VERSION, GraphSchemaError, parse_graph, serialise_graph
...
def make_router(*, templates: Jinja2Templates, ctx: BuilderContext) -> APIRouter:
    ...
    async def save_graph(name: str, request: Request) -> JSONResponse:
        ...
        with ctx.reload_lock:
            store.save_one(g)
            ctx.reload_all_fn()
        return JSONResponse(status_code=200, content={"ok": True, "name": name, "version": CURRENT_SCHEMA_VERSION})
