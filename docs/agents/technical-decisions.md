...
| Schema-driven kwargs form | `ToolEntry.args_meta` + `GET /command/kwargs-form` HTMX fragment + `kwargs_mode=guided\|advanced` POST field | Registry holds schema; dedicated endpoint swaps only the kwargs section; two modes preserve backwards compat | [0056](../decisions/0056-schema-driven-kwargs-form.md) |
