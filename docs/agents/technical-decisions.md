...
| Fuzzy match | ~~`rapidfuzz`~~ — REMOVED | Dropped with the hybrid router; `rapidfuzz` dependency deleted | [0041](../decisions/0041-drop-rapidfuzz-dependency.md) |
...
| Decision | Choice | Why | ADR |
|---|---|---|---|
| Routing strategy | LLM-only — all transcripts routed through `LLMRouter` unconditionally via `Resolver` | Hybrid router removed; single path simplifies architecture and removes rapidfuzz dependency | [0040](../decisions/0040-llm-only-routing-replaces-hybrid.md) |
...
| Resolver entry point | `Resolver` class wraps `LLMRouter.route()` + fires `on_miss()` on `None`; daemon calls `resolver.resolve(text)` | Clean separation: Resolver owns routing decision, LLMRouter owns HTTP, Dispatcher owns execution | [0042](../decisions/0042-resolver-module-design.md) |
| Primitive catalog | Nine verbs: `wait`, `press_keys`, `type_text`, `focus_window`, `launch`, `no_match`, `scroll`, `open_url`, `close_window` | Added scroll, open_url, close_window to cover high-frequency voice intents not expressible with 6 primitives | [0043](../decisions/0043-nine-verb-primitive-catalog.md) |
